import { padNumber, rgbToHex } from '.';
import type { Palette } from 'color-thief-node';
import crypto from 'crypto';

export function toDurationString(ms: number) {
  ...
  return `${h > 0 ? `${padNumber(h)}:` : ''}${padNumber(m)}:${padNumber(s)}`;
}
...
export function paletteToColor(palette: Palette) {
  return hexStringToNumber(rgbToHex(palette[0], palette[1], palette[2]));
}

export function textOverflow(text: string, length: number) {
  return text.length > length ? `${text.substring(0, length - 1)}…` : text;
}

export function joinWithLimit(array: string[], limit: number, text: string, separator = '\n') {
  if (array.length <= limit) {
    return array.join(separator);
  }

  const last = array.length - limit;
  return `${array.slice(0, limit).join(separator)}\n${text.replace('{0}', last.toString())}`;
}

export function isURL(url: string): url is `http://${string}` | `https://${string}` {
...
export function getEmojis(emoji: string) {
  const regex =
